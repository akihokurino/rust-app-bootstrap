...
pub fn my_playground_source(config: GraphQLPlaygroundConfig) -> String {
    let title = "GraphQL Playground";
    r##"
<!DOCTYPE html>
<html>
<head>
...
<body>
  ...
  <script type="text/javascript">
    window.addEventListener('load', function (event) {
      ...
      // Chromeでの自動補完ポップアップ問題を修正
      setTimeout(() => {
        document.addEventListener('click', (e) => {
          // 自動補完の選択肢をクリックした場合
          if (e.target.closest('.CodeMirror-hint')) {
            setTimeout(() => {
              const elements = document.querySelectorAll('.CodeMirror-hints, .CodeMirror-hints-wrapper');
              elements.forEach(el => {
          ...
          // 自動補完の外側をクリックした場合
          else if (!e.target.closest('.CodeMirror-hints-wrapper')) {
            const elements = document.querySelectorAll('.CodeMirror-hints, .CodeMirror-hints-wrapper');
            elements.forEach(el => {
              el.style.display = 'none';
            });
        ...
        document.addEventListener('keydown', (e) => {
          if (e.key === 'Enter' && document.querySelector('.CodeMirror-hints-wrapper')) {
            setTimeout(() => {
              const elements = document.querySelectorAll('.CodeMirror-hints, .CodeMirror-hints-wrapper');
              elements.forEach(el => {
                el.style.display = 'none';
